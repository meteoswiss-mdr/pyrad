...
def main():
    ...
    for trt_fname in trt_list:
        ...
        if args.path_structure in (2, 3):
            ...
            for fbase in file_base2:
                flist.extend(glob.glob(
                    fbase+'PROFILE_'+args.dataset+'/*_'+trt_cell_id+'_rhi_profile_*_' +
        ...
        (traj_ID, yyyymmddHHMM, lon, lat, _, _, _, area, _, _, _, RANKr, _, _,
         _, _, _, _, _, _, _, _, _, _, _, _, _, _) = read_trt_traj_data(
             trt_fname)

        hmin, hmax, freq = get_graupel_column(
            tbin_edges, hbin_edges, data_ma, start_time, yyyymmddHHMM)

        fname = (
            args.trtbase+'rpc_rad4alp/'+trt_cell_id +
            '_cell_rimed_particles_column.csv')
        write_trt_rpc(
            traj_ID, yyyymmddHHMM, lon, lat, area, RANKr, hmin, hmax, freq,
...
def get_graupel_column(tbin_edges, hbin_edges, data_ma, start_time,
    ...
    as the maximum extend where rimed particles (code 5) or solid hail (code 9)
    in the hydrometeor classification are dominant
