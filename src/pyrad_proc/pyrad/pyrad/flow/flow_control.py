...
####
    ...
    if not len(radar_list):
        try:
            ...
            assert 'latitude' in dset['RadarPosition'].keys()
            assert 'longitude' in dset['RadarPosition'].keys()
            assert 'altitude' in dset['RadarPosition'].keys()
        except:
    ...
    # process all data sets
    dscfg, traj = _process_datasets(
        dataset_levels, cfg, dscfg, radar_list, None, traj=traj,
        infostr=infostr)
